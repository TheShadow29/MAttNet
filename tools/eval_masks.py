...
def main(args):
    ...
    results_path = osp.join('cache/results', dataset_splitBy,
                            'dets', args.id+'_'+args.split+'.json')
    ...
    for pid, pred in enumerate(results['predictions']):
        ...
        seg_total += 1
        print('%s/%s expressions evaluated, iou=%.2f.' %
              (pid+1, len(results['predictions']), I*1.0/U))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--id', default='mrcn_cmr_with_st',
                        type=str, help='model id name')
