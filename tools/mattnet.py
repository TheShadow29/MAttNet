...
def show_boxes(img, boxes, colors, texts=None):
    ...
    for k in range(boxes.shape[0]):
        ...
        if texts is not None:
            ax.text(xmin, ymin, texts[k], bbox={
...
class MattNet():
    ...
    def __init__(self, args):
        # load model
        model_prefix = osp.join(
            ROOT_DIR, 'output', args.dataset+'_'+args.splitBy, args.model_id)
        tic = time.time()
    ...
    def forward_image(self, img_path, nms_thresh=.3, conf_thresh=.65):
        ...
        cls_to_dets, num_dets = self.cls_to_detections(
            scores, boxes, nms_thresh, conf_thresh)
        ...
        # 2nd step: get masks
        boxes = xywh_to_xyxy(np.array([det['box']
                                       for det in dets]))  # xyxy (n, 4) ndarray
        labels = np.array(
    ...
    def comprehend(self, img_data, expr):
        ...
        ).numpy().tolist()   # list of 3 weights
        # attributes
    ...
    def fetch_neighbour_ids(self, ref_det_id, Dets):
        ...
        det_ids = sorted(det_ids, cmp=cmp_to_key(compare))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_id', type=str,
                        default='mrcn_cmr_with_st', help='model id name')
