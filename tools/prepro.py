...
def build_vocab(refer, params):
    ...
    print('number of bad words: %d/%d = %.2f%%' % (len(bad_words),
                                                   len(word2count), len(bad_words)*100.0/len(word2count)))
    ...
    sentOrig = {}
    for sent_id, tokens in sentToTokens.items():
        final = [wd if word2count[wd] >
                 count_thr else '<UNK>' for wd in tokens]
        sentToFinal[sent_id] = final
        sentOrig[sent_id] = tokens
    return vocab, sentToFinal, sentOrig
...
def encode_captions(sentences, wtoi, params):
    ...
    LO = np.empty((M, max_length), dtype=f'|S{max_length}')
...
def prepare_json(refer, sentToFinal, ref_to_att_wds, params, sentOrig):
    ...
    for image_id, image in refer.Imgs.items():
        ...
        images += [{'image_id': image_id, 'height': height, 'width': width,
                    'file_name': file_name, 'ref_ids': ref_ids, 'ann_ids': ann_ids, 'h5_id': h5_id}]
        h5_id += 1
    ...
    for image_id in refer.Imgs:
        ...
        for ann_id in ann_ids:
            ...
            h5_id += 1
    print('There are in all %d anns within the %d images.' %
          (h5_id, len(images)))
    ...
    for sent_id, tokens in sentToFinal.items():
        ...
        sent = {'sent_id': sent_id, 'tokens': tokens,
                'h5_id': h5_id, 'full_sent': sentOrig[sent_id]}
...
def build_att_vocab(refer, params, att_types=['r1', 'r2', 'r7']):
    ...
    for sent in sents:
        ...
        for att_type in att_types:
            ...
            if len(att_wds) > 0:
                ref_to_att_wds[ref_id] = ref_to_att_wds.get(
                    ref_id, []) + att_wds
    ref_to_att_wds = {ref_id: list(set(att_wds))
...
def main(params):
    ...
    # create vocab
    vocab, sentToFinal, sentOrig = build_vocab(refer, params)
    ...
    att2cnt, ref_to_att_wds = build_att_vocab(
        refer, params, ['r1', 'r2', 'r7'])
    itoa = {i: a for i, a in enumerate(att2cnt.keys())}
    atoi = {a: i for i, a in enumerate(att2cnt.keys())}

    # prepare refs, images, anns, sentences
    # and write json
    refs, images, anns, sentences = prepare_json(
        refer, sentToFinal, ref_to_att_wds, params, sentOrig)
    dict_to_save = {'refs': refs,
                    'images': images,
                    'anns': anns,
                    'sentences': sentences,
                    'word_to_ix': wtoi,
    ...
    f = h5py.File(osp.join('cache/prepro', dataset +
                           '_'+splitBy, params['output_h5']), 'w')
    L, LO = encode_captions(sentences, wtoi, params)
    f.create_dataset("labels", dtype='int32', data=L)
    f.create_dataset("labels_full", dtype=f"|S{params['max_length']}", data=LO)
    f.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', default='refcoco',
                        type=str, help='refcoco/refcoco+/refcocog')
    parser.add_argument('--splitBy', default='unc',
                        type=str, help='unc/google')
    # refcoco 10, refclef 10, refcocog 20
