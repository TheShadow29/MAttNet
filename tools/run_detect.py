...
def main(args):
    ...
    cat_name_to_cat_ix = {
        category_name: category_id for category_id, category_name in refer.Cats.items()}
    ...
    with torch.no_grad():
        for image_id, image in refer.Imgs.items():
            ...
            cls_to_dets, num_dets = cls_to_detections(
                scores, boxes, imdb, args.nms_thresh, float(args.conf_thresh))
            ...
            while num_dets == 0:
                thresh = thresh-0.1
                cls_to_dets, num_dets = cls_to_detections(
                    scores, boxes, imdb, args.nms_thresh, thresh)
    ...
    # to cache/detections/
    save_path = osp.join(save_dir, '%s_%s_%s_dets.json' %
...
if __name__ == '__main__':
    ...
    parser.add_argument('--conf_thresh', default=0.65,
