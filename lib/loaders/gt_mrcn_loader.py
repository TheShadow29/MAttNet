...
class GtMRCNLoader(Loader):
    ...
    def getBatch(self, split, opt):
        ...
        for image_id in batch_image_ids:
            ...
            for ref_id in ref_ids:
                ...
                pos_sent_ids = self.fetch_sent_ids_by_ref_id(
                    ref_id, seq_per_ref)

                # neg ids
                neg_ann_ids, neg_sent_ids = self.sample_neg_ids(
                    ref_ann_id, seq_per_ref, sample_ratio)
            ...
            # fetch feats
            pos_ann_boxes = xywh_to_xyxy(
                np.vstack([self.Anns[ann_id]['box'] for ann_id in image_pos_ann_ids]))
            image_pos_pool5, image_pos_fc7 = self.fetch_grid_feats(
                pos_ann_boxes, head, im_info)  # (num_pos, k, 7, 7)
            batch_pos_pool5 += [image_pos_pool5]
            batch_pos_fc7 += [image_pos_fc7]
            neg_ann_boxes = xywh_to_xyxy(
                np.vstack([self.Anns[ann_id]['box'] for ann_id in image_neg_ann_ids]))
            image_neg_pool5, image_neg_fc7 = self.fetch_grid_feats(
                neg_ann_boxes, head, im_info)  # (num_neg, k, 7, 7)
        ...
        pos_fc7 = torch.cat(batch_pos_fc7, 0)
        pos_fc7.detach()
        pos_pool5 = torch.cat(batch_pos_pool5, 0)
        pos_pool5.detach()
        pos_lfeats = self.compute_lfeats(batch_pos_ann_ids)
        pos_dif_lfeats = self.compute_dif_lfeats(batch_pos_ann_ids)
        pos_labels = np.vstack([self.fetch_seq(sent_id)
                                for sent_id in batch_pos_sent_ids])
        neg_fc7 = torch.cat(batch_neg_fc7, 0)
        neg_fc7.detach()
        neg_pool5 = torch.cat(batch_neg_pool5, 0)
        neg_pool5.detach()
        neg_lfeats = self.compute_lfeats(batch_neg_ann_ids)
        neg_dif_lfeats = self.compute_dif_lfeats(batch_neg_ann_ids)
        neg_labels = np.vstack([self.fetch_seq(sent_id)
                                for sent_id in batch_neg_sent_ids])

        # fetch cxt_fc7 and cxt_lfeats
        pos_cxt_fc7, pos_cxt_lfeats, pos_cxt_ann_ids = self.fetch_cxt_feats(
            batch_pos_ann_ids, opt)
        neg_cxt_fc7, neg_cxt_lfeats, neg_cxt_ann_ids = self.fetch_cxt_feats(
            batch_neg_ann_ids, opt)
    ...
    def sample_neg_ids(self, ann_id, seq_per_ref, sample_ratio):
        ...
        """
        st_ref_ids, st_ann_ids, dt_ref_ids, dt_ann_ids = self.fetch_neighbour_ids(
            ann_id)
        ...
        for i, ref_ann_id in enumerate(ann_ids):
            ...
            for j, cand_ann_id in enumerate(st_ann_ids[:topK]):
                ...
                dif_lfeats[i, j*5:(j+1)*5] = \
                    np.array([(cx1-rcx)/rw, (cy1-rcy)/rh,
    ...
    def fetch_cxt_feats(self, ann_ids, opt):
        ...
        topK = opt['num_cxt']
        cxt_feats = np.zeros(
            (len(ann_ids), topK, self.fc7_dim), dtype=np.float32)
    ...
    def get_attribute_weights(self, scale=10):
        ...
        cnts = [self.att_to_cnt[self.ix_to_att[ix]]
                for ix in range(self.num_atts)]
        cnts = np.array(cnts)
        weights = 1/cnts**0.5
    ...
    def getAttributeBatch(self, split):
        ...
        ann_boxes = xywh_to_xyxy(
            np.vstack([self.Anns[ann_id]['box'] for ann_id in ann_ids]))
        pool5, fc7 = self.fetch_grid_feats(
            ann_boxes, head, im_info)  # (#ann_ids, k, 7, 7)
        ...
        data['Feats'] = {'pool5': pool5, 'fc7': fc7,
                         'lfeats': lfeats, 'dif_lfeats': dif_lfeats}
        data['bounds'] = {'it_pos_now': self.iterators[split],
                          'it_max': max_index, 'wrapped': wrapped}
        return data
