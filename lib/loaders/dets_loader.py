...
class DetsLoader(Loader):
    ...
    def fetch_cxt_feats(self, det_ids, opt):
        ...
        topK = opt['num_cxt']
        cxt_feats = np.zeros(
            (len(det_ids), topK, self.fc7_dim), dtype=np.float32)
    ...
    def getTestBatch(self, split, opt):
        ...
        det_boxes = xywh_to_xyxy(
            np.vstack([self.Dets[det_id]['box'] for det_id in det_ids]))
        pool5, fc7 = self.fetch_grid_feats(
            det_boxes, head, im_info)  # (#det_ids, k, 7, 7)
