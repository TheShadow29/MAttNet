...
def eval_attributes(loader, model, split, opt):
    ...
    while True:
        ...
        # forward sub_encoder
        Feats = data['Feats']
        fake_phrase_emb = Variable(torch.zeros(
            len(data['ref_ids']), opt['word_vec_size']).float().cuda())
        _, _, att_scores = model.sub_encoder(
            Feats['pool5'], Feats['fc7'], fake_phrase_emb)
    ...
    while True:
        ...
        for i, sent_id in enumerate(sent_ids):
            ...
            for k in top_att_ixs:
                pred_atts.append(
                    (loader.ix_to_att[k], float(pred_att_scores[k])))

            # add info
            entry = {}
            entry['sent_id'] = sent_id
            entry['sent'] = loader.decode_labels(label.data.cpu().numpy())[
                0]  # gd-truth sent
            entry['gd_ann_id'] = data['ann_ids'][gd_ix]
            entry['pred_ann_id'] = data['ann_ids'][pred_ix]
            entry['pred_score'] = scores.tolist()[pred_ix]
            entry['sub_grid_attn'] = sub_grid_attn[pred_ix].data.cpu(
            ).numpy().tolist()  # list of 49 attn
            entry['sub_attn'] = sub_attn[pred_ix].data.cpu(
            ).numpy().tolist()  # list of seq_len attn
            entry['loc_attn'] = loc_attn[pred_ix].data.cpu(
            ).numpy().tolist()  # list of seq_len attn
            entry['rel_attn'] = rel_attn[pred_ix].data.cpu(
            ).numpy().tolist()  # list of seq_len attn
            # rel ann_id
            entry['rel_ann_id'] = data['cxt_ann_ids'][pred_ix][rel_ix]
            entry['weights'] = weights[pred_ix].data.cpu(
            ).numpy().tolist()   # list of 2 weights
...
def eval_dets_split(loader, model, crit, split, opt):
    ...
    while True:
        ...
        for i, sent_id in enumerate(sent_ids):
            ...
            # rel det_id
            entry['rel_det_id'] = data['cxt_det_ids'][pred_ix][rel_ix]
            entry['weights'] = weights[pred_ix].data.cpu(
            ).numpy().tolist()   # list of 2 weights
            entry['pred_atts'] = pred_atts  # list of (att_wd, score)
            predictions.append(entry)
