...
class Matching(nn.Module):
    ...
    def forward(self, visual_input, lang_input):
        ...
        # l2-normalize
        visual_emb_normalized = nn.functional.normalize(
            visual_emb, p=2, dim=1)  # (n, jemb_dim)
        lang_emb_normalized = nn.functional.normalize(
            lang_emb, p=2, dim=1)     # (n, jemb_dim)

        # compute cossim
        cossim = torch.sum(visual_emb_normalized *
                           lang_emb_normalized, 1)  # (n, )
...
class RelationMatching(nn.Module):
    ...
    def forward(self, visual_input, lang_input, masks):
        ...
        visual_emb = self.vis_emb_fc(
            visual_input.view(n*m, -1))  # (n x m, jemb_dim)
        ...
        # l2-normalize
        visual_emb_normalized = nn.functional.normalize(
            visual_emb, p=2, dim=1)  # (nxm, jemb_dim)
        lang_emb_normalized = nn.functional.normalize(
            lang_emb, p=2, dim=1)   # (nxm, jemb_dim)

        # compute cossim
        cossim = torch.sum(visual_emb_normalized *
                           lang_emb_normalized, 1)  # (nxm, )
...
class JointMatching(nn.Module):
    ...
    def forward(self, pool5, fc7, lfeats, dif_lfeats, cxt_fc7, cxt_lfeats, labels):
        ...
        sub_feats, sub_grid_attn, att_scores = self.sub_encoder(
            pool5, fc7, sub_phrase_emb)  # (n, fc7_dim+att_dim), (n, 49), (n, num_atts)
        sub_matching_scores = self.sub_matching(
            sub_feats, sub_phrase_emb)  # (n, 1)
