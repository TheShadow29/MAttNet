    ...
    def forward(self, input_labels):
        ...
        if self.variable_lengths:
            ...
            sorted_input_lengths_list = np.sort(input_lengths_list)[
                ::-1].tolist()  # list of sorted input_lengths
            ...
            recover_ixs = input_labels.data.new(
                recover_ixs).long()  # Variable long

            # sort input_labels by descending order
            input_labels = input_labels[sort_ixs]

        # embed
        # (n, seq_len, word_embedding_size)
        ...
        if self.variable_lengths:
            ...
            if self.rnn_type == 'lstm':
                ...
                hidden = hidden[0]
            # (num_layers * num_dirs, batch, hidden_size)
            hidden = hidden[:, recover_ixs, :]
            # (batch, num_layers * num_dirs, hidden_size)
...
class PhraseAttention(nn.Module):
    ...
    def forward(self, context, embedded, input_labels):
        ...
        # (batch, seq_len)
        attn = attn / attn.sum(1).view(attn.size(0),
                                       1).expand(attn.size(0), attn.size(1))
