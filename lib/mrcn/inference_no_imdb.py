...
class Inference:
    ...
    def __init__(self, args):
        ...
        # Config
        cfg_file = osp.join(
            mrcn_dir, 'experiments/cfgs/%s.yml' % self.net_name)
        cfg_list = ['ANCHOR_SCALES', [4, 8, 16, 32],
    ...
    def predict(self, img_path):
        ...
        _, scores, bbox_pred, rois, _ = self.net.test_image(
        ...
        if cfg.TEST.BBOX_REG:
            ...
            pred_boxes = bbox_transform_inv(torch.from_numpy(
                boxes), torch.from_numpy(box_deltas)).numpy()
    ...
    def extract_head(self, img_path):
        ...
        im_info = np.array(
            [[blobs['data'].shape[1], blobs['data'].shape[2], im_scales[0]]])
    ...
    def head_to_prediction(self, net_conv, im_info):
        ...
        if cfg.TRAIN.BBOX_NORMALIZE_TARGETS_PRECOMPUTED:
            stds = bbox_pred.data.new(cfg.TRAIN.BBOX_NORMALIZE_STDS).repeat(
                self.num_classes).unsqueeze(0).expand_as(bbox_pred)
    ...
    def box_to_spatial_fc7(self, net_conv, im_info, ori_boxes):
        ...
        # (n, 2048, 7, 7), equavalent to _head_to_tail
        spatial_fc7 = self.net.resnet.layer4(pool5)
    ...
    def spatial_fc7_to_prediction(self, spatial_fc7, im_info, ori_boxes):
        ...
        # make rois
        batch_inds = Variable(spatial_fc7.data.new(
            ori_boxes.shape[0], 1).zero_())
        scaled_boxes = (ori_boxes * im_info[0][2]).astype(np.float32)
        scaled_boxes = Variable(torch.from_numpy(scaled_boxes).cuda())
        rois = torch.cat([batch_inds, scaled_boxes], 1)
